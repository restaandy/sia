function tampildataekstra(e,base_url){
 waitingDialog.show('Loading');	
 var id=$(e.target).attr('data-id');
 $.post(base_url+'modal/modal_ekstra',{id:id},function(data){
  $("#dataekstra .modal-body").html(data);
  waitingDialog.hide();
  $("#dataekstra").modal("show");
 });
}
...
$(function() {
        ...
        $("#ekstra_siswa").autocomplete({
            source: function(term, suggest){
                $.post('http://localhost/sia/ekstra/autocomplete_ekstra',{autocomplete:'yes',value:$('#ekstra_siswa').val()},function(data){
                    suggest(JSON.parse(data));
                });
            },
            focus: function(event, ui ) {
                $('#ekstra_siswa').val(ui.item.label);
                return false;
            }
        });
